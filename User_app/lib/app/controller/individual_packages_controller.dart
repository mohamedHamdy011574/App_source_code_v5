...
class IndividualPackagesController extends GetxController implements GetxService {
  ...
  Future<void> getSalonDetails() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (Get.find<ServiceCartController>().checkPackageInCart(packages.id as int)) {
      ...
      if (packagesDetails.images != 'NA' && packagesDetails.images != null && packagesDetails.images != '') {
  ...
  void addPackageToCart() {
    if (Get.find<ServiceCartController>().savedInCart.packages!.isEmpty && Get.find<ServiceCartController>().savedInCart.services!.isEmpty) {
      _packagesDetails.isBooked = true;
      Get.find<ServiceCartController>().addPackageToCart(_packagesDetails, 'salon');
      update();
    } else if (Get.find<ServiceCartController>().savedInCart.services!.isNotEmpty) {
      int freenlancerServiceId = Get.find<ServiceCartController>().getServiceFreelancerId();
      if (freenlancerServiceId == _packagesDetails.uid) {
        _packagesDetails.isBooked = true;
        Get.find<ServiceCartController>().addPackageToCart(_packagesDetails, 'individual');
        update();
      } else {
        showToast('We already have service or package with other salon or with freelancer'.tr);
      }
    } else {
      int freelancerId = Get.find<ServiceCartController>().getPackageFreelancerId();
  ...
  void removePackageFromCart() {
    ...
    Get.find<ServiceCartController>().removePackageFromCart(_packagesDetails.id as int);
    Get.find<HomeController>().updateScreen();
    update();
  }
