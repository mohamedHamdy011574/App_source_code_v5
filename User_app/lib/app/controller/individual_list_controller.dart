...
class IndividualListController extends GetxController implements GetxService {
  ...
  Future<void> getSalonDetails() async {
    var response = await parser.getServicesById({"id": selectedServiceId, "uid": selectedUID});
    ...
    if (response.statusCode == 200) {
      ...
      body.forEach((data) {
        ...
        if (Get.find<ServiceCartController>().checkServiceInCart(services.id as int)) {
  ...
  void updateServiceStatusInCart(int index, bool status) {
    ...
    if (Get.find<ServiceCartController>().savedInCart.services!.isEmpty && Get.find<ServiceCartController>().savedInCart.packages!.isEmpty) {
      ...
      if (status == true) {
        Get.find<ServiceCartController>().addServiceToCart(_servicesList[index], 'individual');
      } else {
        Get.find<ServiceCartController>().removeServiceFromCart(_servicesList[index].id as int);
      }
    } else if (Get.find<ServiceCartController>().savedInCart.packages!.isNotEmpty) {
      int freelancerPackagesId = Get.find<ServiceCartController>().getPackageFreelancerId();
      ...
      int freelancerIdServices = Get.find<ServiceCartController>().getServiceFreelancerId();
