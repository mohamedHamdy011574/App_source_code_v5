...
class ProductsDetailsParser {
  ...
  ProductsDetailsParser({required this.apiService, required this.sharedPreferencesManager});

  Future<Response> getProductsInfo(var body) async {
    var response = await apiService.postPrivate(AppConstants.getProductsInfo, body, sharedPreferencesManager.getString('token') ?? '');
    return response;
  }
