...
class OrderDetailsParser {
  ...
  Future<Response> getAppointmentDetails(var body) async {
    var response = await apiService.postPrivate(AppConstants.getAppointmnetsDetails, body, sharedPreferencesManager.getString('token') ?? '');
    return response;
  }

  Future<Response> updateAppointments(var body) async {
    var response = await apiService.postPrivate(AppConstants.updateAppointments, body, sharedPreferencesManager.getString('token') ?? '');
    return response;
  }
